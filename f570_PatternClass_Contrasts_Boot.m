function repdata=f570_PatternClass_Contrasts_Boot(hmiconfig,cm_all,cm_trials,neuron_filter,neuron_label,rep_number)
% function to conduct Pattern Classifier Analysis
% as per M.Stokes, script by AHB, June 2012
% Updated October 2012, for paper
% Meant to limit number of neurons in sample population (randomly selected)
...
disp('%%% Bootstrapping n=X neuron data...')

disp(['.....Sampling from ',num2str(rep_number),' neurons...'])
repdata=zeros(bootst,13,60);
for rr=1:bootst,
    disp(['...Repetition ',num2str(rr)]);
    unit_pointer=randperm(nNeurons,rep_number);
    for unit=1:length(unit_pointer), % Create correlation matrices for sample neuronal population
        un=unit_pointer(unit);
    ...
    repdata(rr,:,:)=diagonalsNT.Fish;

end
save([hmiconfig.faces570spks,filesep,neuron_label,num2str(rep_number),'-570repdata.mat'],'repdata')
