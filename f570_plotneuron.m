...
[bardata rep_range]=f570_calcAdapt_winxp(respstruct.trial_m_epoch1',respstruct.trial_id(:,6));
hold on
errorbar(1:rep_range,bardata(:,1),bardata(:,2),'k-','LineWidth',2)
for c=1:3,
    pointer=find(respstruct.trial_id(:,5)==c);
    [bar(c).data bar(c).range]=f570_calcAdapt_winxp(respstruct.trial_m_epoch1(pointer)',respstruct.trial_id(pointer,6));
end
...
errorbar(1:bar(1).range,bar(1).data(:,1),bar(1).data(:,2),'k-')
errorbar(1:bar(2).range,bar(2).data(:,1),bar(2).data(:,2),'r-')
errorbar(1:bar(3).range,bar(3).data(:,1),bar(3).data(:,2),'b-')
legend('Neutral','Threat','Grin')
xlabel('Number of Presentations','FontSize',fontsize_med); ylabel('Average Firing Rate (sp/s)','FontSize',fontsize_med);
set(gca,'FontSize',fontsize_med); title({'Stimulus Adaptation'},'FontSize',10,'FontWeight','Bold'); % if unable to sync
...
hold on
for c=1:2,
    pointer=find(respstruct.trial_id(:,4)==c);
    [bar(c).data bar(c).range]=f570_calcAdapt_winxp(respstruct.trial_m_epoch1(pointer)',respstruct.trial_id(pointer,6));
end
errorbar(1:bar(1).range,bar(1).data(:,1),bar(1).data(:,2),'k-')
errorbar(1:bar(2).range,bar(2).data(:,1),bar(2).data(:,2),'r-')
