function f570_socialcue_response_nonface(neuron_type)
global xldata monkeyname hmiconfig

%disp('Figure X - Non-face responsive neurons')
figure; clf; cla; %
set(gcf,'Units','Normalized','Position',[0.05 0.15 0.5 0.7]); set(gca,'FontName','Arial','FontSize',10);
%%% FACIAL EXPRESSION %%%
fepointerS=find(ismember(xldata.confprefcat,neuron_type)==1 & xldata.validface>=0.05 & xldata.anova_fe'<0.05);
fepointerNS=find(ismember(xldata.confprefcat,neuron_type)==1 & xldata.validface>=0.05 & xldata.anova_fe'>=0.05);
[avgfunc, semfunc]=f570_avg_spden_nonface(xldata,fepointerS,hmiconfig,hmiconfig.xrange,monkeyname);
subplot(3,2,1); hold on % sig, norm, fe
plx_shade_spden(hmiconfig.xrange(1):hmiconfig.xrange(2),avgfunc(9,:),semfunc(9,:),0.25,'k') % neutral normalised 
plx_shade_spden(hmiconfig.xrange(1):hmiconfig.xrange(2),avgfunc(11,:),semfunc(11,:),0.25,'r') % threat normalised 
plx_shade_spden(hmiconfig.xrange(1):hmiconfig.xrange(2),avgfunc(13,:),semfunc(13,:),0.25,'b') % fear grin normalised 
...
%set(gca,'FontName','Arial','FontSize',hmiconfig.fontsize_sml); ylim([0 20]); xlim([-100 400]);
...
title({'(A) Facial Expression Non Face Neurons, Directed Only)',['Sig Neurons (n=',num2str(length(fepointerS)),')']},'FontSize',hmiconfig.fontsize_lrg)
%legend('N','','T','','Fg','')
...
ylabel('Average Response (sp/s)','FontSize',hmiconfig.fontsize_med); ylim([0.5 2])
set(gca,'XTickLabel',{'N-s','T-s','FG-s','N-ns','T-ns','FG-ns'},'FontSize',hmiconfig.fontsize_sml,'XTick',1:6);
...
axis square;
xlim([0 6]); ylim([0 6]); plot([0 6],[0 6],'k:')
plot([0 1],[1 1],'k:'); plot([1 1],[0 1],'k:')
title('(D) Compare Threat vs. Fear Grin','FontSize',10)
subplot(3,3,7); piedata=[]; %
piedata(1)=length(find(ismember(xldata.confprefcat,neuron_type)==1 & xldata.validface>=0.05 & xldata.anova_fe'<0.05));
piedata(2)=length(find(ismember(xldata.confprefcat,neuron_type)==1 & xldata.validface>=0.05 & xldata.anova_fe'>=0.05));
pie(piedata,...
    {['n=',num2str(piedata(1)),'(',num2str(piedata(1)/sum(piedata)*100,'%1.2g'),'%)'] ...
    ['n=',num2str(piedata(2)),'(',num2str(piedata(2)/sum(piedata)*100,'%1.2g'),'%)']})
title([monkeyname,' - ',neuron_type,' Neurons - FE (n=',num2str(sum(piedata)),')'],'FontSize',hmiconfig.fontsize_lrg,'FontWeight','Bold')
legend('S','NS','Location','Best'); set(gca,'FontSize',hmiconfig.fontsize_sml)
subplot(3,3,8); clear piedata
bardataS=xldata.expr_rsp_avg(fepointerS,:);
...
jpgfigname=[hmiconfig.figurepath,filesep,'Faces570_Fig7pt1_nonrespFacialExpression.jpg'];
print(gcf,jpgfigname,'-djpeg') % generates an JPEG file of the figure
export_fig Faces570_Fig7pt1_nonrespFacialExpression.eps -eps -transparent -rgb

figure; clf; cla; %
set(gcf,'Units','Normalized','Position',[0.05 0.15 0.5 0.7]); set(gca,'FontName','Arial','FontSize',10);
%%% GAZE DIRECTION %%%
disp('Figure 2C,D - Effect of Facial Expression on Face Responses')
gdpointerS=find(ismember(xldata.confprefcat,neuron_type)==1 & xldata.validface>=0.05 & xldata.anova_fe'<0.05);
gdpointerNS=find(ismember(xldata.confprefcat,neuron_type)==1 & xldata.validface>=0.05 & xldata.anova_fe'>=0.05);
[avgfunc, semfunc]=f570_avg_spden_nonface(xldata,gdpointerS,hmiconfig,hmiconfig.xrange,monkeyname);
...
%set(gca,'FontName','Arial','FontSize',hmiconfig.fontsize_sml); ylim([0 20]); xlim([-100 400]);
xlabel('Time from stimulus onset (ms)','FontSize',hmiconfig.fontsize_med); ylabel('Normalized Firing Rate*','FontSize',hmiconfig.fontsize_med)
title({'(C) Gaze Direction Nonface Neurons, Neutral Only)',['Sig Neurons (n=',num2str(length(gdpointerS)),')']},'FontSize',hmiconfig.fontsize_lrg)
%legend('D','','A','')
...
axis square;
xlim([0 30]); ylim([0 30]); plot([0 80],[0 80],'k:')
% plot([0 1],[1 1],'k:'); plot([1 1],[0 1],'k:')
title('(D) Compare Directed vs. Averted','FontSize',10)

subplot(3,3,7); piedata=[]; %
piedata(1)=length(find(ismember(xldata.confprefcat,neuron_type)==1 & xldata.validface>=0.05 & xldata.anova_gd'<0.05));
piedata(2)=length(find(ismember(xldata.confprefcat,neuron_type)==1 & xldata.validface>=0.05 & xldata.anova_gd'>=0.05));
pie(piedata,...
    {['n=',num2str(piedata(1)),'(',num2str(piedata(1)/sum(piedata)*100,'%1.2g'),'%)'] ...
    ['n=',num2str(piedata(2)),'(',num2str(piedata(2)/sum(piedata)*100,'%1.2g'),'%)']})
title([monkeyname,' - ',neuron_type,' Neurons - GD (n=',num2str(sum(piedata)),')'],'FontSize',hmiconfig.fontsize_lrg,'FontWeight','Bold')
legend('S','NS','Location','Best'); set(gca,'FontSize',hmiconfig.fontsize_sml)
...
jpgfigname=[hmiconfig.figurepath,filesep,'Faces570_Fig7pt2_nonrespGazeDirection.jpg'];
print(gcf,jpgfigname,'-djpeg') % generates an JPEG file of the figure
export_fig Faces570_Fig7pt2_nonrespGazeDirection.eps -eps -transparent -rgb

%%% IDENTITY %%%
disp('Figure 2E,F - Effect of Identity on Face Responses')
figure; clf; cla; %
set(gcf,'Units','Normalized','Position',[0.05 0.15 0.5 0.7]); set(gca,'FontName','Arial','FontSize',10);
idpointerS=find(ismember(xldata.confprefcat,neuron_type)==1 & xldata.validface>=0.05 & xldata.anova_id'<0.05);
idpointerNS=find(ismember(xldata.confprefcat,neuron_type)==1 & xldata.validface>=0.05 & xldata.anova_id'>=0.05);
...
xlabel('Time from stimulus onset (ms)','FontSize',hmiconfig.fontsize_med); ylabel('Normalized Firing Rate*','FontSize',hmiconfig.fontsize_med)
title({'(E)Identity Non Face Neurons - Neutral,Directed Only)',['Sig Neurons (n=',num2str(length(idpointerS)),')']},'FontSize',hmiconfig.fontsize_lrg)
...
pie(piedata,...
    {['n=',num2str(piedata(1)),'(',num2str(piedata(1)/sum(piedata)*100,'%1.2g'),'%)'] ...
    ['n=',num2str(piedata(2)),'(',num2str(piedata(2)/sum(piedata)*100,'%1.2g'),'%)']})
title({'Non face neurons - Identity',['(n=',num2str(sum(piedata)),')']},'FontSize',hmiconfig.fontsize_med,'FontWeight','Bold')
