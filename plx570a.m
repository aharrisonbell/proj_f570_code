function plx570(files);
%%%%%%%%%%%%%%%%%%%%%%%%
% plx570(files); %
%%%%%%%%%%%%%%%%%%%%%%%%
% written by AHB, July 2009,
% Based on plx500_quick, for use in recording sessions
...
disp('*********************************************************************')
disp('plx570.m - Analysis program for FACES570-series datafiles (July 2009)')
disp('*********************************************************************')
        ...
        for un=1:foundunits, % performed for each unit
            ...
            for tr=1:size(tempspike(un).spikes,1),
                ...
                switch respstructsingle.trial_id(tr,1)
                    case {1 2 3 4 5 6 7 8} % Identity 1
                        respstructsingle.trial_id(tr,3)=1;
                    case {9 10 11 12 13 14 15 16} % Identity 2
                        respstructsingle.trial_id(tr,3)=2;
                    case {17 18 19 20 21 22 23 24} % Identity 3
                        respstructsingle.trial_id(tr,3)=3;
                    case {25 26 27 28 29 30 31 32} % Identity 4
                        respstructsingle.trial_id(tr,3)=4;
                    case {33 34 35 36 37 38 39 40} % Identity 5
                        respstructsingle.trial_id(tr,3)=5;
                    case {41 42 43 44 45 46 47 48} % Identity 6
                        respstructsingle.trial_id(tr,3)=6;
                    case {49 50 51 52 53 54 55 56} % Identity 7
                        respstructsingle.trial_id(tr,3)=7;
                    case {57 58 59 60 61 62 63 64} % Identity 8
            ...
            %%% graph the file
            disp('...Graphing data...')
            plotneuron(hmiconfig,xscale,graphstructsingle,respstructsingle,char(files(f)))
